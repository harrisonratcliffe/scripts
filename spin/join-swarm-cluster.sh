...
if [[ "$IS_MANAGER" == "y" || "$IS_MANAGER" == "Y" || "$IS_MANAGER" == "yes" || "$IS_MANAGER" == "YES" ]]; then
    read -p "Enter the Swarm cluster token (you can get the token from the manager with \"sudo docker swarm join-token manager | grep 'docker swarm join --token' | awk '{print \$5}'\"): " SWARM_TOKEN
else
    read -p "Enter the Swarm cluster token (you can get the token from the manager with \"sudo docker swarm join-token worker | grep 'docker swarm join --token' | awk '{print \$5}'\"): " SWARM_TOKEN
fi
